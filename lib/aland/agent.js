...
function buildSystemPrompt(config, knowledgeContext, serviceName) {
  return [
    ...
    '',
    'BASE DE CONOCIMIENTO (usa esta información; si pregunta cuántos/qué servicios hay, responde con el catálogo):',
    knowledgeContext || '(Sin información adicional cargada)',
    '',
    'Si hay bloque de CONTENIDO PÚBLICO DEL SITIO, úsalo. NUNCA inventes ni explores rutas de administración (/admin, /ops-*, paneles internos).',
    '',
...
async function generateAlandReply({ appStore, conversation, userMessage, config }) {
  ...
  // Si hay links públicos (o el usuario pregunta por el sitio), leer solo páginas públicas
  try {
    const publicWeb = require('./publicWeb');
    const webBits = await publicWeb.gatherPublicWebContext([
      userMessage,
      ...(knowledge || []).filter((k) => k.sourceType === 'custom').map((k) => `${k.title}\n${k.content}`)
    ]);
    if (webBits) {
      knowledgeContext = `${knowledgeContext}\n\nCONTENIDO PÚBLICO DEL SITIO (nunca uses rutas admin):\n${webBits}`;
    }
