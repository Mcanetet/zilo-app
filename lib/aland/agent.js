...
async function generateAlandReply({ appStore, conversation, userMessage, config }) {
  ...
  return {
    reply,
    escalate,
    escalateReason: check.reason || 'ai',
    securityAlert
  };
}
...
async function escalateToAdmin(conversation, io, reason = 'timeout') {
  ...
  const message = await alandStore.addMessage({
    ...
    body: reason === 'timeout'
      ...
      : reason === 'security'
        ? 'Alerta de seguridad: posible intento de prompt injection o acceso indebido. Conversación escalada a administración.'
...
async function processClientMessage({ appStore, io, conversationId, user, text }) {
  ...
  const { reply, escalate, securityAlert } = await generateAlandReply({
    appStore,
    conversation,
    userMessage: text,
    config
