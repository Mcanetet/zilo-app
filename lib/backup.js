...
function startBackupScheduler(store, logEvent) {
  ...
  setInterval(() => {
    ...
    try {
      ...
      console.log(`[Zilo Backup] Completado: ${result.results.length} copia(s), ${result.removed} antigua(s) eliminada(s)`);
    } catch (err) {
      saveConfig({ lastBackupStatus: 'error', lastBackupError: err.message });
      console.error('[Zilo Backup] Error:', err.message);
    }
  }, 60 * 1000);
}
...
function getRetentionSummary(config) {
  ...
  return {
    ...
    marketReference: {
      daily: '7–30 días (Zilo: 14 por defecto)',
      weekly: '4–12 semanas (Zilo: 8 por defecto)',
      monthly: '12–36 meses (Zilo: 12 por defecto)',
