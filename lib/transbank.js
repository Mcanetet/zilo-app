...
function buildBuyOrder(requestId) {
  // Solo caracteres permitidos y últimos 26 para que un ajuste no colisione
  // con la orden de la visita (la referencia adicional termina en su propio UUID).
  return String(requestId).replace(/[^a-zA-Z0-9]/g, '').slice(-26);
}
...
async function createTransaction({ request, baseUrl }) {
  ...
  const buyOrder = buildBuyOrder(reference);
  const sessionId = String(reference).slice(0, 61);
  const chargeQuery = request.additionalChargeId
    ? `&charge=${encodeURIComponent(request.additionalChargeId)}`
    : '';
  const returnUrl = `${baseUrl}/pagos/transbank/retorno?ref=${encodeURIComponent(request.id)}${chargeQuery}`;
